...
from create_csv import write_emodb_csv, write_tess_ravdess_csv, write_custom_csv, write_asvp_esd_csv
...
class EmotionRecognizer:
    ...
    def __init__(self, model=None, **kwargs):
        ...
        Params:
            ...
            emodb (bool): whether to use EMO-DB Speech dataset, default is False,
            asvp_esd (bool): whether to use ASVP_ESD Speech dataset, default is False,
            custom_db (bool): whether to use custom Speech dataset that is located in `data/train-custom`
                and `data/test-custom`, default is False
            ...
            asvp_esd_name (str): the name of the output CSV file for EMO-DB dataset, default is "asvp_esd.csv"
        ...
        Note that when all databases are set to `False`, `tess_ravdess` will be set to True
        automatically.
        """
        # emotions
        ...
        self.asvp_esd = kwargs.get("asvp_esd", True)
        self.custom_db = kwargs.get("custom_db", False)

        if not self.tess_ravdess and not self.emodb and not self.asvp_esd and not self.custom_db:
        ...
        self.asvp_esd_name = kwargs.get("asvp_esd_name", "asvp_esd.csv")
    ...
    def _set_metadata_filenames(self):
        ...
        if self.asvp_esd:
            train_desc_files.append(f"train_{self.asvp_esd_name}")
            test_desc_files.append(f"test_{self.asvp_esd_name}")
        if self.custom_db:
    ...
    def write_csv(self):
        ...
        for train_csv_file, test_csv_file in zip(self.train_desc_files, self.test_desc_files):
            ...
            elif self.asvp_esd_name in train_csv_file:
                write_asvp_esd_csv(self.emotions, train_name=train_csv_file, test_name=test_csv_file, verbose=self.verbose)
                if self.verbose:
                    print("[+] Generated ASVP-ESD DB CSV File")
...
def plot_histograms(classifiers=True, beta=0.5, n_classes=3, verbose=1):
    ...
    for estimator, params, cv_score in estimators:
        ...
        for i in range(3):
            ...
            result['f_train'] = detector.train_fbeta_score(beta=beta)
            result['f_test'] = detector.test_fbeta_score(beta=beta)
            if verbose:
