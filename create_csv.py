...
def write_emodb_csv(emotions=["sad", "neutral", "happy"], train_name="train_emo.csv",
    ...
    for emotion, code in categories_reversed.items():
        if emotion not in emotions:
            del categories[code]
    for file in glob.glob(f"{ROOT_DIR}/data/emodb/wav/*.wav"):
        try:
...
def write_tess_ravdess_csv(emotions=["sad", "neutral", "happy"], train_name="train_tess_ravdess.csv",
    ...
    for category in emotions:
        ...
        total_files = glob.glob(f"{ROOT_DIR}/data/training/Actor_*/*_{category}.wav")
        ...
        total_files = glob.glob(f"{ROOT_DIR}/data/validation/Actor_*/*_{category}.wav")
        for i, path in enumerate(total_files):
...
def write_asvp_esd_csv(emotions=["sad", "neutral", "happy"], train_name="train_asvp_esd.csv",
                       test_name="test_asvp_esd.csv", train_size=0.8, verbose=1):
    """
    Reads speech ASVP-ESD dataset from directory and write it to a metadata CSV file.
    params:
        emotions (list): list of emotions to read from the folder, default is ['sad', 'neutral', 'happy']
        train_name (str): the output csv filename for training data, default is 'train_asvp_esd.csv'
        test_name (str): the output csv filename for testing data, default is 'test_asvp_esd.csv'
        train_size (float): the ratio of splitting training data, default is 0.8 (80% Training data and 20% testing data)
        verbose (int/bool): verbositiy level, 0 for silence, 1 for info, default is 1
    """
    target = {"path": [], "emotion": []}
    ...
    for emotion, code in categories_reversed.items():
        if emotion not in emotions:
            del categories[code]
    for file in glob.glob(f"{ROOT_DIR}/data/ASVP-ESD-Update/Audio/actor*/*.wav"):
        try:
            emotion = categories[os.path.basename(file)[7]]
        except KeyError:
            continue
        target['emotion'].append(emotion)
        target['path'].append(file)
    if verbose:
        print("[ASVP-ESD] Total files to write:", len(target['path']))
    ...
    train_size = int(train_size * n_samples)
    if verbose:
        print("[ASVP-ESD] Training samples:", train_size)
        print("[ASVP-ESD] Testing samples:", test_size)
    X_train = target['path'][:train_size]
    X_test = target['path'][train_size:]
    y_train = target['emotion'][:train_size]
    y_test = target['emotion'][train_size:]
...
def write_custom_csv(emotions=['sad', 'neutral', 'happy'], train_name="train_custom.csv", test_name="test_custom.csv",
    ...
    for category in emotions:
        # train data
        for i, file in enumerate(glob.glob(f"{ROOT_DIR}/data/train-custom/*_{category}.wav")):
        ...
        # test data
        for i, file in enumerate(glob.glob(f"{ROOT_DIR}/data/test-custom/*_{category}.wav")):
