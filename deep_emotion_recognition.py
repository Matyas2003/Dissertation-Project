...
from keras.optimizers import rmsprop_legacy
from keras.layers import LSTM, GRU, Dense, Activation, LeakyReLU, Dropout, MaxPooling1D, Flatten
...
class DeepEmotionRecognizerRNN(EmotionRecognizer):
    """
...
class DeepEmotionRecognizerCNN(EmotionRecognizer):
    """
    The Deep Learning version of the Emotion Recognizer.
    This class uses CNN (Conv1d, MaxPooling, etc.).
    """
    ...
    def __init__(self, **kwargs):
        ...
        self.n_cnn_layers = kwargs.get("n_cnn_layers", 2)
        self.cnn_units = kwargs.get("cnn_units", 128)
        # self.cell = kwargs.get("cell", LSTM)
    ...
    def _update_model_name(self):
        ...
        # 'c' for classification & 'r' for regression
        problem_type = 'c' if self.classification else 'r'
        self.model_name = f"CNN-{emotions_str}-{problem_type}-layers-{self.n_cnn_layers}-units-{self.cnn_units}.h5"
    ...
    def create_model(self):
        ...
        opt = rmsprop_legacy.RMSprop(lr=0.00005, rho=0.9, epsilon=1e-07, decay=0.0)
    ...
    def train(self, override=False):
        ...
        self.checkpointer = ModelCheckpoint(model_filename, save_best_only=True, verbose=1)
        self.tensorboard = TensorBoard(log_dir=os.path.join("logs", self.model_name))
        print(self.X_test.shape, self.y_train.shape, self.X_test.shape, self.y_test.shape)
        self.history = self.model.fit(self.X_train, self.y_train,
                                      batch_size=self.batch_size,
                                      epochs=self.epochs,
                                      validation_data=(self.X_test, self.y_test),
    ...
    def confusion_matrix(self, percentage=True, labeled=True):
        ...
        # invert from keras.utils.to_categorical
        y_test = np.array([np.argmax(y, axis=None, out=None) for y in self.y_test[0]])
        matrix = confusion_matrix(y_test, y_pred, labels=[self.emotions2int[e] for e in self.emotions]).astype(
            np.float32)
        if percentage:
            for i in range(len(matrix)):
                matrix[i] = matrix[i] / np.sum(matrix[i])
...
if __name__ == "__main__":
    rec = DeepEmotionRecognizerRNN(emotions=['angry', 'sad', 'neutral', 'happy'],
